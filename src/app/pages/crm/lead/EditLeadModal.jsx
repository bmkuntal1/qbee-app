...
const EditLeadModal = ({ id, show, onClose }) => {
    const { data: customList } = useQuery({
        queryKey: ['customListEditLead'], queryFn: getCustomList, refetchOnWindowFocus: false, select: groupCustomList
    });

    const { isLoading: isLeadLoading, data: leadData } = useQuery({
        queryKey: ['editLead', id], 
        queryFn: () => getData(id),
        select: (data) => { return { ...data, isAccount: data.isAccount ? 'true' : 'false' } }
    });
    ...
    const onSubmit = (data) => {
        ...
        mutate({ ...data, isAccount:watchIsAccount=='true', id });
    }

    return (
        <PageModal title="Edit Lead" show={show} onClose={onClose} className={!isSuccess && "modal-lg"}>
            {isError && <div className="alert alert-danger">{error?.response?.data?.toString()}</div>}
            {isSuccess ? <ResultAlert type="success" message="Lead updated successfully" /> :
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="row mx-2">
                        ...
                        <div className="col-md-6">
                            <FormField label="Source" type="react-select" control={control} name="source" options={customList?.LeadSource} register={register} errors={errors} />
                        </div>
                        <div className="col-md-6">
                            <FormField label="Industry" type="react-select" control={control} name="industry" options={customList?.Industry} register={register} errors={errors} />
                        </div>
                        <div className="col-md-12">
